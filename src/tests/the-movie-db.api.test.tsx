...
import { fetchSingleMovie, fetchSingleMovieCredit, fetchTopRated } from "../apis/the-movie-db.api";
...
describe('testing axios request fetchSingleMovie', () => {
    test('WHEN fetchSingleMovie BY id THEN return expected data MovieDetail', async () => {
        ...
        expect(axios.get).toHaveBeenCalledWith('https://api.themoviedb.org/3/movie/ID?api_key=92b418e837b833be308bbfb1fb2aca1e');
...
describe('testing axios request fetchSingleMovieCredit', () => {
    test('WHEN fetchSingleMovieCredit BY id THEN return expected data MovieCredit', async () => {
        ...
        expect(axios.get).not.toHaveBeenCalled();
        const response: AxiosResponse<MovieCredit> = await fetchSingleMovieCredit('ID');
        expect(axios.get).toHaveBeenCalledTimes(1);
        expect(axios.get).toHaveBeenCalledWith('https://api.themoviedb.org/3/movie/ID/credits?api_key=92b418e837b833be308bbfb1fb2aca1e');
        expect(response.data).toBe(expectedData);
    })
...
describe('testing axios request fetchTopRated', () => {
    test('WHEN fetchTopRated BY id THEN return expected data Movie[]', async () => {
        ...
        expect(axios.get).not.toHaveBeenCalled();
        const response: AxiosResponse<{ results: Movie[] }> = await fetchTopRated(1);
        expect(axios.get).toHaveBeenCalledTimes(1);
        expect(axios.get).toHaveBeenCalledWith('https://api.themoviedb.org/3/movie/top_rated?api_key=92b418e837b833be308bbfb1fb2aca1e&page=1');
